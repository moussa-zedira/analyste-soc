...
class PanoramaConnector(Connector):
    ...
    async def block_ip(self, ip: str, address_group: str = "SOAR-Blocklist") -> dict[str, Any]:
        ...
        r1 = await self._api({"type": "config", "action": "set", "xpath": xpath_obj, "element": element_obj})
        if not r1["ok"]:
        ...
        r2 = await self._api({"type": "config", "action": "set", "xpath": xpath_grp, "element": element_grp})
        if not r2["ok"]:
    ...
    async def block_domain(self, fqdn: str, address_group: str = "SOAR-Domains-Blocklist") -> dict[str, Any]:
        if not self.configured:
            return {"applied": False, "reason": "panorama_not_configured"}
