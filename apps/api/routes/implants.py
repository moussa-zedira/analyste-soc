...
class ShellcodeRequest(BaseModel):
    payload_type: str = Field(
        "reverse_tcp", description="reverse_tcp, bind_tcp, exec_cmd, download_exec"
    )
    target_os: str = "linux"
    arch: str = "x64"
    lhost: str = "0.0.0.0"
    ...
    output_format: str = Field(
        "hex", description="raw, c_array, python, powershell, csharp, hex, base64"
    )
...
class LoaderRequest(BaseModel):
    ...
    technique: str = Field(
        "virtualalloc", description="virtualalloc, apc, hollowing, callback, fiber"
    )


class EvasionRequest(BaseModel):
    technique: str = Field(
        ..., description="amsi_bypass, etw_patch, unhook_ntdll, direct_syscall, process_inject"
    )
...
class PivotRequest(BaseModel):
    pivot_type: str = Field(
        "socks5", description="socks5, port_forward_local, port_forward_remote, ssh_tunnel, chisel"
    )
...
async def build_implant(body: ImplantBuildRequest):
    ...
    try:
        ...
        return {
            ...
            "estimated_size": result.estimated_size_kb,
...
def list_templates():
    return {
        "templates": [
            {
                "language": "go",
                "name": "Go HTTP Implant",
                "description": "Full-featured Go implant, cross-compiles to EXE/ELF/Mach-O",
                "features": [
                    "sysinfo",
                    "cmd",
                    "upload",
            ...
            {
                "language": "c",
                "name": "C Minimal Implant",
                "description": "Tiny C implant ~10KB, minimal footprint",
                "features": ["cmd", "sysinfo"],
                "evasion_score": 9,
            },
            {
                "language": "rust",
                "name": "Rust Memory-Safe Implant",
                "description": "Rust implant, hard to reverse engineer",
                "features": [
                    "sysinfo",
                    "cmd",
                    "upload",
            ...
            },
            {
                ...
                "description": "In-memory PowerShell with AMSI/ETW bypass",
                "features": ["cmd", "amsi_bypass", "etw_patch", "clm_bypass"],
                "evasion_score": 6,
            },
            {
                "language": "python",
                "name": "Python Cross-Platform",
                "description": "Python implant, PyInstaller packaging",
                "features": [
                    "sysinfo",
                    "cmd",
                    "upload",
        ...
        "protocols": [
            {
                "id": "http",
                "name": "HTTP/HTTPS",
                "description": "Standard web callback with malleable profiles",
                "stealth": 7,
            },
            ...
            {"id": "icmp", "name": "ICMP", "description": "ICMP payload data exfil", "stealth": 8},
            {
                "id": "websocket",
                "name": "WebSocket",
                "description": "Persistent WS connection",
                "stealth": 6,
            },
            {
                "id": "smb",
                "name": "SMB Named Pipe",
                "description": "Lateral movement friendly, no internet",
                "stealth": 8,
            },
            {
                "id": "doh",
                "name": "DNS over HTTPS",
                "description": "Uses Google/Cloudflare DoH",
                "stealth": 10,
            },
        ]
    }
...
def list_tunnels():
    return {
        "tunnels": [],
        "message": "No active tunnels. Start one via POST /pivot/socks5 or /pivot/port-forward.",
    }
