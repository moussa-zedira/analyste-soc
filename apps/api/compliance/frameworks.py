...
ISO_27001 = Framework(
    ...
    controls=[
        Control("iso.a5.7", "Threat intelligence",
                "Veille sur les menaces",
                ["ioc.feeds", "mitre.coverage"]),
        Control("iso.a5.24", "Information security incident management",
                "Planification de la gestion des incidents",
                ["incident.playbook", "case_mgmt.sla"]),
        Control("iso.a5.30", "ICT readiness for BC",
                "Continuite TIC",
                ["backup.dr"]),
        Control("iso.a8.7", "Protection against malware",
                "Protection contre les malwares",
                ["detection.malware", "ioc.feeds"]),
        Control("iso.a8.8", "Management of technical vulnerabilities",
                "Gestion des vulns techniques",
                ["vuln.scan"]),
        Control("iso.a8.15", "Logging",
                "Journalisation",
                ["sigma.rules", "audit.immutable"]),
        Control("iso.a8.16", "Monitoring activities",
                "Surveillance des activites",
                ["uba.profiling", "detection.brute_force"]),
        Control("iso.a8.24", "Use of cryptography",
                "Cryptographie",
                ["encryption.tls", "secrets.vault"]),
    ],
)
...
NIST_CSF = Framework(
    ...
    controls=[
        Control("csf.id.am", "Asset Management",
                "Inventaire des assets",
                ["vuln.scan"]),
        Control("csf.id.ra", "Risk Assessment",
                "Evaluation des risques",
                ["mitre.coverage", "vuln.scan"]),
        Control("csf.pr.aa", "Identity Mgmt & Access Control",
                "Gestion identite + controle acces",
                ["audit.user_actions"]),
        Control("csf.pr.ds", "Data Security",
                "Protection des donnees",
                ["encryption.tls", "secrets.vault"]),
        Control("csf.de.ae", "Anomalies & Events",
                "Detection des anomalies",
                ["uba.profiling", "sigma.rules"]),
        Control("csf.de.cm", "Continuous Monitoring",
                "Monitoring continu",
                ["sigma.rules", "audit.immutable"]),
        Control("csf.rs.an", "Analysis",
                "Analyse des incidents",
                ["case_mgmt.sla", "mitre.coverage"]),
        Control("csf.rs.mi", "Mitigation",
                "Confinement + eradication",
                ["incident.playbook"]),
        Control("csf.rc.rp", "Recovery Planning",
                "Planning de recuperation",
                ["backup.dr"]),
    ],
)
...
ALL_FRAMEWORKS: dict[str, Framework] = {
    f.id: f for f in [NIS2, DORA, PCI_DSS, ISO_27001, NIST_CSF]
}
